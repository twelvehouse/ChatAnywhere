...
function PersonalFilterIcon() {
  return <Lock size={10} strokeWidth={2.5} aria-hidden />;
}

function SharedFilterIcon() {
  return <Globe size={10} strokeWidth={2} aria-hidden />;
}
    ...
    <button className={clsx(styles['dm-row'], isActive && styles['dm-active'])} onClick={onSelect}>
      ...
      <div className={styles['dm-info']}>
        ...
        <span className={styles['dm-preview']}>
          {isOutgoing && (
            <CornerDownLeft className={styles['dm-reply-icon']} strokeWidth={2.5} aria-hidden />
          )}
