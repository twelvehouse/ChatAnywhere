...
export async function addGfdStylesheet(gfdPath: string, texPath: string) {
  ...
  const ruleCount = Object.values(stylesheets).length;
  console.log(
    `[GFD] Processed ${gfd.length} entries from ${gfdPath}, generated ${ruleCount} rules.`,
  );
...
async function loadTexAsBlob(path: string) {
  const texRes = await fetch(path);
  if (!texRes.ok) throw new Error(`Failed to fetch ${path}: ${texRes.status}`);
  const tex = parseTex(await texRes.arrayBuffer());
  if (tex.format !== 0x1450)
    // B8G8R8A8
    throw new Error('Not supported format for TEX');

  const dataArray = new Uint8ClampedArray(
    tex.buffer,
    tex.offsetToSurface[0],
...
async function loadGfd(path: string) {
  const gfdRes = await fetch(path);
  if (!gfdRes.ok) throw new Error(`Failed to fetch ${path}: ${gfdRes.status}`);
  const buffer = new DataView(await gfdRes.arrayBuffer());
  const count = buffer.getInt32(8, true);
  const entries: GdfEntry[] = new Array(count);
  for (let i = 0; i < count; i++) {
    const offset = 0x10 + i * 0x10;
    entries[i] = {
      id: buffer.getUint16(offset, true),
      ...
      redirect: buffer.getUint16(offset + 12, true),
...
function parseTex(arrayBuffer: ArrayBuffer) {
  ...
  const lodOffsets = [
    buffer.getInt32(16, true),
    buffer.getInt32(20, true),
    buffer.getInt32(24, true),
  ];
  const offsetToSurface = [
    buffer.getInt32(28, true),
    buffer.getInt32(32, true),
    buffer.getInt32(36, true),
    Math.max(0, buffer.getInt32(40, true)),
    Math.max(0, buffer.getInt32(44, true)),
    Math.max(0, buffer.getInt32(48, true)),
    Math.max(0, buffer.getInt32(52, true)),
    Math.max(0, buffer.getInt32(56, true)),
    Math.max(0, buffer.getInt32(60, true)),
    Math.max(0, buffer.getInt32(64, true)),
    Math.max(0, buffer.getInt32(68, true)),
    Math.max(0, buffer.getInt32(72, true)),
    Math.max(0, buffer.getInt32(76, true)),
  ];
